---
name: captions
description: Use when the user wants to transcribe a video, an audio file, or a folder of them into text. Captions is a .NET console tool that uses OpenAI Whisper (speech-to-text) to produce a single combined Markdown transcript and/or one plain-text file per source file. Triggers include requests like "transcribe these videos", "transcribe this audio", "get the text of this recording", or "generate a transcript from this footage".
---

# Captions

Captions transcribes one or more video or audio files into text using a local Whisper model. It
is a .NET 10 console application. ffmpeg and the Whisper model are downloaded automatically on
first run, so the only hard prerequisite is the .NET SDK.

## When to use this skill

Use it whenever the goal is to turn the audio of a video or audio file (or a batch of them in a
folder) into a written transcript. After running it, the produced `transcriptions.md` is a good
input for the `notary` agent, which converts transcripts into detailed software requirements.
...
- **Combined file (default):** `<out-dir>/transcriptions.md` — one `##` heading per source title
  followed by its transcription. This is the canonical artifact to hand to the `notary` agent.
- **Per-file output (`--out-each`):** `<out-dir>/<source-name>.txt` — the plain transcription of
